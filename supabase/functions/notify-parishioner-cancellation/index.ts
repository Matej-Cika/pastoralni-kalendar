/**
 * notify-parishioner-cancellation
 *
 * Called when a PRIEST manually cancels a confirmed booking.
 * Sends an email to the parishioner explaining the cancellation,
 * including the priest's reason.
 *
 * Deploy via Supabase Dashboard → Edge Functions → New function
 *   Name: notify-parishioner-cancellation
...
function buildEmailHtml(opts: {
  ...
  const { parishionerName, priestName, date, startTime, endTime, purpose, cancellationReason } = opts
      ...
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 20px rgba(0,0,0,.07);">
        <tr>
          <td style="background:linear-gradient(135deg,#dc2626,#b91c1c);padding:28px 40px;text-align:center;">
            ...
            <h1 style="margin:8px 0 4px;font-size:20px;font-weight:700;color:#fff;">Vaša rezervacija je otkazana</h1>
        ...
        <tr>
          <td style="padding:32px 40px;">
            ...
            <p style="margin:0 0 28px;font-size:15px;color:#374151;line-height:1.6;">
              župnik <strong>${priestName}</strong> otkazao je sljedeći zakazani susret:
            </p>
            ...
            <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:12px;padding:16px 20px;margin-bottom:24px;">
              <p style="margin:0 0 6px;font-size:12px;font-weight:600;color:#92400e;text-transform:uppercase;letter-spacing:.05em;">Razlog otkazivanja</p>
              <p style="margin:0;font-size:14px;color:#78350f;line-height:1.6;">${cancellationReason}</p>
            </div>` : ''}
            <p style="margin:0;font-size:14px;color:#6b7280;line-height:1.6;">
              Za više informacija ili novi termin, molimo kontaktirajte župni ured.
            </p>
          </td>
        </tr>
        <tr>
...
serve(async (req) => {
  ...
  try {
    ...
    if (booking.cancelled_by !== 'PRIEST') {
    ...
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        subject: `Otkazivanje susreta – ${formatCroatianDate(slot.date)}`,
        html,
      }),
    })
    ...
    await supabase.from('email_log').insert({
      email_type: 'priest_cancels_parishioner', recipient_email: parishioner.email,
    ...
    console.error('notify-parishioner-cancellation error:', err)
